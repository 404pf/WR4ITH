...
def ask_claude(messages: list, system: str) -> str:
    ...
    if not key or len(key) < 20:
        return "[!] No valid API key. Add ANTHROPIC_API_KEY to .env or ~/.wr4ith_key"
    ...
    try:
        resp = requests.post(ANTHROPIC_API_URL, headers=headers, json=payload, timeout=180)
        ...
        if code == 400: return "[!] Bad request — context may be too large."
        ...
        return "[!] No connection to Claude API — check internet."
...
def dispatch(call_type: str, call_content: str, target: str) -> str:
    ...
    if call_type == "TOOL":
        ...
        passive = call_content.strip().lower() == "passive"
...
def run_all_dispatches(response: str, target: str) -> str:
    ...
    for call_type, call_content in calls:
        output = dispatch(call_type, call_content, target)
...
def parse_vulnerabilities(response: str) -> list:
    ...
    while i < len(lines):
        ...
        if line.startswith("VULN:"):
            vuln = {"vuln_name": "", "severity": "medium", "port": "", "service": "", "description": "", "poc": "", "fix": ""}
...
---
TOOL SYNTAX:
  [WEBRECON: passive]      — passive web recon (headers, JS, robots, fingerprint)
  [WEBRECON: active]       — full web recon including path probing
  [TOOL: nmap -sV target]  — run a network tool
  [TOOL: dig A target]     — DNS lookup
  [SEARCH: CVE-2021-xxxxx] — CVE or vulnerability search

Follow the recon decision tree from Section 3.
...
Follow your recon decision tree. Use [WEBRECON:], [TOOL:], or [SEARCH:] to request data.
I will run each and return results."""
    }]
    ...
    for loop in range(MAX_LOOPS):
        ...
        messages.append({
            "role": "user",
            "content": f"[TOOL RESULTS]\n{tool_results}\n\nContinue. Produce final report in Section 6 format when done."
        })
